...
| **Auto-Import** | Enabled (no annotation) |
...
| File | Purpose |
|------|---------|
| `hostedcluster.yaml` | Defines hosted cluster (auto-import enabled) |
...
ACM discovers the HostedCluster and automatically creates ManagedCluster.
...
2. Lets ACM auto-import and create ManagedCluster
...
**HostedCluster (no auto-import annotation):**
```bash
cat hostedcluster.yaml | grep -i "managed\|annotation"
```

Expected: No annotation disabling auto-import
...
- T+10m: ACM auto-creates ManagedCluster
...
2. HostedCluster auto-imports to ACM
3. ACM auto-creates ManagedCluster
...
1. Remove ManagedCluster YAML
2. Remove KlusterletAddonConfig YAML
3. Remove auto-import disable annotation from HC
4. Add ClusterSet, Placement, Policy YAML
5. Commit and sync
6. MCE auto-creates ManagedCluster
7. Placement selects it
8. Policies applied

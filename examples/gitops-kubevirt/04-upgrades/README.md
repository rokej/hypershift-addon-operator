...
- Base state: 4.16.1 for both HC and NodePool
- Upgrade 1: Control plane 4.16.1 → 4.17.0 (without upgrading nodes)
...
- Upgrade 4: Rollback example (revert to 4.16.1 if needed)

## Prerequisites

Complete the setup in `../setup/`:
- [ ] ACM/MCE installed with HyperShift addon
- [ ] Argo CD installed and configured
- [ ] KubeVirt configured
- [ ] Secrets created (`pull-secret`, `ssh-key`)
...
├── base/
│   ├── hostedcluster.yaml               # Base HC (4.16.1)
│   └── nodepool.yaml                    # Base NodePool (4.16.1)
├── upgrades/
│   ├── 01-control-plane-upgrade.yaml    # HC 4.16.1 → 4.17.0
│   ├── 02-nodepool-upgrade-replace.yaml # NodePool Replace strategy
│   ├── 03-nodepool-upgrade-rolling.yaml # NodePool InPlace strategy
│   └── 04-rollback-example.yaml         # Revert to 4.16.1
└── monitoring/
    ├── check-upgrade-status.sh          # Real-time upgrade monitor
    └── upgrade-conditions.md            # Status reference guide
```
...
First, deploy a running cluster at 4.16.1:
...
Expected: Both HC and NodePool healthy, version 4.16.1
...
Now upgrade the HostedCluster from 4.16.1 to 4.17.0:
...
# Change image: quay.io/openshift-release-dev/ocp-release:4.16.1-x86_64
#         to: quay.io/openshift-release-dev/ocp-release:4.17.0-x86_64
# Commit and push
```
...
# Or manually watch:
oc get hostedcluster -n clusters example-hcp -w
oc get clusterversion  # Shows control plane version
oc get pods -n clusters-example-hcp -w  # Watch control plane components
```
...
# Desired version should be 4.17.0
```
...
# Change release.image to 4.17.0
# Commit and push
```
...
export KUBECONFIG=/tmp/example-hcp-kubeconfig
oc get nodes -o wide
# All nodes should show 4.17.0 in STATUS
...
3. Commit changes
   git add .
   git commit -m "chore: upgrade to 4.17.0"

4. Create PR
   git push origin upgrade-4.17
   # Create pull request
...
10. Close PR / Archive branch
    git branch -d upgrade-4.17
```
...
```bash
# Start monitoring (continuous updates every 5 seconds)
./monitoring/check-upgrade-status.sh
...
POLL_INTERVAL=10 ./monitoring/check-upgrade-status.sh  # 10 seconds

# Run for limited time
MAX_ITERATIONS=120 ./monitoring/check-upgrade-status.sh  # ~10 minutes
```

The script shows:
- Control plane version and status
- NodePool upgrade progress
...
```bash
# Edit base/nodepool.yaml
# Revert image back to 4.16.1
oc apply -f upgrades/02-nodepool-upgrade-replace.yaml  # Uses 4.16.1
# or
...
Minor version upgrades (4.17.0 → 4.17.1) can be done more frequently.

## Next Steps

- [Scenario 1: Provisioning](../01-provision/) - Review base provisioning
- [Scenario 3: Scaling](../03-scaling/) - Review scaling patterns
- [Documentation: Upgrade Strategy](../../docs/gitops/05-upgrades.md)
